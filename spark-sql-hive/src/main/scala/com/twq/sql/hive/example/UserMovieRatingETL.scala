...
object UserMovieRatingETL {
  ...
  def doETL(spark: SparkSession): Unit = {
    val basePath = spark.conf.get("spark.user.movie.rating.basePath", "src/test/resources/ml-100k")
    ...
    val movieDF = spark.read
      ...
      .toDF("movie_id", "movie_title", "release_date", "video_release_date", "imdb_url", "unknown",
    ...
    movieDF.select($"movie_id", $"movie_title", $"release_date", $"video_release_date",
